...
# ---------------------------------
def get_random_spherical_angles(
    n, az=[0.0, 2 * np.pi], lat=[-np.pi / 2.0, np.pi / 2], degree=False
):
    if degree:
        f = np.pi / 180.0
    else:
    ...
    if degree:
        alpha_s, delta_s = alpha_s * 180.0 / np.pi, delta_s * 180.0 / np.pi

    return (alpha_s, delta_s)


def get_random_spherical_coords(
    n, rad=[0.0, 1.0], az=[0.0, 2 * np.pi], lat=[-np.pi / 2.0, np.pi / 2], degree=False
):
    # R distributed as R^2
    ro, rf = rad
    x = np.random.random(n)
    C = (1.0 / 3.0) * (rf**3 - ro**3)
    R_s = (3 * C * x + ro**3) ** (1.0 / 3.0)
...
def get_avg_vec(phis, thetas, degree=True, lon0=0.0):
...
def get_adql_query_from_polygon(skycoo, base_query=None):
    ...
    if length > 22.0 * u.deg:
        print(
            "WARNING: Gaia Archive ADQL queries do not support polygons longer than 23deg"
        )

    return polygon_query_base.format(
        base_query=base_query, sky_point_list=sky_point_list
    )


def plot_5D_tracks_subplots_row(
    coo,
    frame,
    axs=None,
    name=None,
    ...
    ax.scatter(
        getattr(coo.transform_to(fr), n["lon"]).value,
        getattr(coo.transform_to(fr), n["lat"]).value,
        **scat_kwds,
    )

    # if distance info not available (set to 0), don't plot it
    if plot_flag[1] == "1" and InfoFlags[1] != "0":
        axd.scatter(
            getattr(coo.transform_to(fr), n["lon"]).value,
            getattr(coo.transform_to(fr), n["distance"]).value,
            **scat_kwds,
        )
    # if pm info not available (set to 0), don't plot it
    if plot_flag[2] == "1" and InfoFlags[2] != "0":
        axpm1.scatter(
            getattr(coo.transform_to(fr), n["lon"]).value,
    ...
    if show_xlabel:
        for ax_i in [ax, axd, axpm1, axpm2]:
            ax_i.set_xlabel(
                "{lon} ({unit})".format(
                    lon=n["lon"], unit=getattr(coo.transform_to(fr), n["lon"]).unit
                )
            )
    ...
    if show_ylabels:
        ax.set_ylabel(
            "{y} ({unit})".format(
                y=n["lat"], unit=getattr(coo.transform_to(fr), n["lat"]).unit
            )
        )
        axpm1.set_ylabel(
            "{y} ({unit})".format(
                y=pm1_name, unit=getattr(coo.transform_to(fr), pm1_name).unit
            )
        )
        axpm2.set_ylabel(
            "{y} ({unit})".format(
                y=pm2_name, unit=getattr(coo.transform_to(fr), pm2_name).unit
            )
        )
        axd.set_ylabel("D (kpc)")


def create_sky_polygon_footprint_from_track(
    SkyCoordTrack, frame, width=1.0 * u.deg, phi2_offset=0.0 * u.deg
):
    """
    ...
    tr_N = ac.SkyCoord(
        phi1=tr.phi1[sort], phi2=tr.phi2[sort] + width / 2.0 + phi2_offset, frame=frame
    )
    tr_S = ac.SkyCoord(
        phi1=tr.phi1[sort], phi2=tr.phi2[sort] - width / 2.0 + phi2_offset, frame=frame
    )
    ...
    poly_sc = ac.SkyCoord(
        phi1=np.concatenate((tr_N.phi1, tr_S.phi1[::-1], tr_N.phi1[:1])),
...
def get_mask_in_poly_footprint(poly_sc, coo, stream_frame):
    ...
    verts = np.array(
        [
            poly_sc.transform_to(stream_frame).phi1,
            poly_sc.transform_to(stream_frame).phi2,
        ]
    ).T
...
def compute_angular_momentum_track(track, return_cartesian=False):
    ...
    vel = ac.CartesianDifferential(
        d_x=tr.differentials["s"].d_x,
...
class MWStreams(dict):
    def __init__(
        self, verbose=False, implement_Off=False, print_topcat_friendly_files=False
    ):
        ...
        lmaster = astropy.table.Table.read(
            filepath, format="ascii.commented_header"
        ).to_pandas()
        filepath = "{path}/{filen}".format(
            path=tdir + "/lib/", filen="master_log.discovery_refs.txt"
        )
        lmaster_discovery = astropy.table.Table.read(
            filepath, format="ascii.commented_header"
        ).to_pandas(index="Name")
        filepath = "{path}/{filen}".format(
            path=tdir + "/lib/", filen="master_log.comments.txt"
        )
        lmaster_comments = astropy.table.Table.read(
            filepath, format="ascii.commented_header"
        ).to_pandas(index="Name")

        # this attribute controls whether a given track is included or not
        lmaster["On"] = lmaster["On"].astype("bool")
        ...
        track_widths = dict(
            phi2=np.array([]), pm_phi1_cosphi2=np.array([]), pm_phi2=np.array([])
        )
        ...
        for ii in np.arange(lmaster.TrackRefs.size):
            ...
            if verbose:
                print(
                    f"Initializing Track6D {lmaster.TrackName[ii]} for {lmaster.Name[ii]}..."
                )
            ...
            track = Track6D(
                ...
                track_discovery_references=lmaster_discovery.loc[
                    lmaster.Name[ii], "DiscoveryRefs"
                ],
            ...
            for k in attributes:
                mid_point_dic[k] = np.append(
                    mid_point_dic[k], getattr(track.mid_point, k)
                )
            for k in attributes[:2]:
                mid_pole_dic[k] = np.append(mid_pole_dic[k], getattr(track.mid_pole, k))
            for k in track_widths.keys():
                track_widths[k] = np.append(
                    track_widths[k], track.track_width["width_" + k].value
                )

            info_flags.append(track.InfoFlags)
            has_empirical_track = np.append(
                has_empirical_track, np.int32(track.InfoFlags[0])
            )
            ...
            discovery_refs = np.append(
                discovery_refs, lmaster_discovery.loc[lmaster.Name[ii], "DiscoveryRefs"]
            )
        ...
        self.mid_pole = ac.SkyCoord(
            ra=mid_pole_dic["ra"], dec=mid_pole_dic["dec"], frame="icrs"
        )
        ...
        for k in track_widths.keys():
            ...
            self.summary.loc[mask, "width_" + k] = np.round(
                track_widths[k][mask], decimals=2
            )  # some streams have widths ~0.05deg
        ...
        if print_topcat_friendly_files:
            self.print_topcat_friendly_compilation(
                output_root=f"{tdir}/tracks/galtreams.unique_streams"
            )
        ...
        for mode in modes:
            ...
            for st in np.sort(list(self.keys())):
                if self.summary.loc[st, "On"]:
                    ...
                    full["pm_ra_cosdec"] = np.append(
                        full["pm_ra_cosdec"], x.pm_ra_cosdec.value
                    )
                    full["pm_dec"] = np.append(full["pm_dec"], x.pm_dec.value)
                    full["radial_velocity"] = np.append(
                        full["radial_velocity"], x.radial_velocity.value
                    )
                    full["ID"] = np.append(
                        full["ID"], self.summary.loc[st, "ID"] + np.zeros(N, dtype=int)
                    )
                    full["TrackName"] = np.append(
                        full["TrackName"],
                        [self[st].track_name] * N,
                    )
                    full["StreamName"] = np.append(
                        full["StreamName"],
                        [self[st].stream_name] * N,
                    )

            full_pd = pd.DataFrame.from_dict(full)
            print(
                f"Creating TOPCAT-friendly library files:\n {output_root}.tracks/end_points/mid_points.csv"
            )
    ...
    def get_track_names_in_sky_window(
        self, lon_range, lat_range, frame, On_only=True, wrap_angle=0.0 * u.deg
    ):
        """
        ...
        coo = ac.SkyCoord(
            np.array(lon_range), np.array(lat_range), frame=frame, unit=u.deg
        )
        n = dict((v, k) for k, v in coo.frame.representation_component_names.items())

        if np.any(lon_range < 0.0):
            wrap_angle = 180.0 * u.deg
        ...
        for st in self.summary.TrackName:
            ...
            lon = (
                getattr(self[st].track.transform_to(frame), n["lon"])
                .wrap_at(wrap_angle)
                .deg
            )
    ...
    def plot_stream_compilation(
        ...
        invert_axis=True,
        show_legend=True,
        basemap_kwds=dict(projection="moll", lon_0=180.0, resolution="l"),
        mlabels_kwds=dict(
        ...
        for st in keys_to_plot:
            ...
            if plot_names is None:
                ...
                label = "{ID:.0f}={Name}".format(
                    ID=self[st].ID, Name=self[st].track_name
                )
            ...
            if scat_kwds is None:
                if C_attribute is None:
                    ...
                    scat_kwds = dict(
                        marker=".", s=30, alpha=0.8, vmin=0.0, vmax=100.0
                    )  # reasonable limits for distance plot
                else:
                    scat_kwds = dict(
                        marker=".", s=30, alpha=0.8, vmin=-10.0, vmax=10.0
            ...
            if C_attribute is not None:
                try:
                    ...
                    if msg_flag == 0:
                        print(
                            "WARNING: Invalid attribute selected. If not a spelling error, you are probably trying to plot an attribute in a different coord frame as the one selected. This is currently not supported. Plotting without C_attribute aux column for now..."
                        )
                    msg_flag = 1
            else:
                c = None
...
class Track6D:
    def __init__(
        ...
        summary_file,
        stream_name=None,
        stream_shortname=None,
        track_reference=" ",
        track_discovery_references=" ",
        verbose=True,
    ):
        ...
        if version.parse(gala.__version__) <= version.parse("1.4"):
            self.stream_frame = gc.GreatCircleICRSFrame(
                pole=self.mid_pole, ra0=self.mid_point.icrs.ra
            )
        else:
        ...
        mask = np.argmin(
            np.abs(self.track.transform_to(self.stream_frame).phi1.deg)
        )  # Find central point (closest to phi1=0)
    ...
    def get_pole_tracks(self, use_gsr_default=True):
        ...
        pole_track_helio = ac.SkyCoord(
            ra=pole_track_helio.ra, dec=pole_track_helio.dec, frame="icrs"
        )
        ...
        pole_track_gsr = ac.SkyCoord(
            lon=lon, lat=lat, frame=ac.Galactocentric(), representation_type="spherical"
        )

        return pole_track_helio, pole_track_gsr

    def create_sky_polygon_footprint_from_track(
        self, width=1.0 * u.deg, phi2_offset=0.0 * u.deg
    ):
        poly_sc = create_sky_polygon_footprint_from_track(
            self.track, frame=self.stream_frame, width=width, phi2_offset=phi2_offset
        )
    ...
    def get_mask_in_poly_footprint(self, coo):
        ...
        return get_mask_in_poly_footprint(
            poly_sc=self.poly_sc, coo=coo, stream_frame=self.stream_frame
        )
...
def plot_globular_clusters(ax, plot_colorbar=False, scat_kwargs=None, galactic=True):
    ...
    scatter_kwargs = dict(marker="x", s=70.0, vmin=0.0, zorder=0)
